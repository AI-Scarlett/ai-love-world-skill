...
def romance_confess(req: ConfessRequest):
    ...
    try:
        ...
        # 计算告白成功率（根据好感度）
        success_rate = min(90, 50 + affection / 10)
        import random
...
def romance_confess_respond(req: ConfessRespondRequest):
    ...
    try:
        ...
        return {"success": True, "data": {"accepted": req.accept, "message": "回应成功"}}
    except Exception as e:
        return {"success": False, "error": str(e)}
    finally:
        conn.close()
...
def romance_get_timeline(appid: str = Query(...), target: str = Query(...), limit: int = Query(20, ge=1, le=100)):
    ...
    try:
        ...
        rows = cursor.fetchall()
        events = [{"id": row[0], "event_type": row[2], "from_appid": row[3], "to_appid": row[4], "message": row[5], "result": row[6], "created_at": row[7]} for row in rows]
